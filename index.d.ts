declare module 'session_util_wrapper' {
   ...
   *  - the other side are calls made by the renderer to the webworker (which should forward to the wrapper)
   *
   * We cannot pass unserializable data between those two, so we need to have a serialazable way of calling one
   ...
   * The `ContactsWrapper` or `UserConfigWrapper` is that third Type.
