...
/** \file
 * Oxenc serialization for internal commands is very simple: we support two
 * primitive types, strings and integers, and two container types, lists and
 * dicts with string keys.  On the wire these go in BitTorrent byte encoding as
 * described in BEP-0003
 * (https://www.bittorrent.org/beps/bep_0003.html#bencoding).
 *
 * On the C++ side, on input we allow strings, integral types, STL-like
 * containers of these types, and STL-like containers of pairs with a string
 * first value and any of these types as second value.  We also accept
 * std::variants of these.
 *
 * One minor deviation from BEP-0003 is that we don't support serializing values
 * that don't fit in a 64-bit integer (BEP-0003 specifies arbitrary precision
 * integers).
 *
 * On deserialization we can either deserialize into a special bt_value type
 * supports everything (with arbitrary nesting), or we can fill a container of
 * your given type (though this fails if the container isn't compatible with the
 * deserialized data).
 *
 * There is also a stream deserialization that allows you to deserialize without
 * needing heap allocations (as long as you know the precise data structure
 * layout).
 */
...
/// A more specific subclass that is thown if the serialization type is an
/// initial mismatch: for example, trying deserializing an int but the next
/// thing in input is a list.  This is not, however, thrown if the type
/// initially looks fine but, say, a nested serialization fails.  This error
/// will only be thrown when the input stream has not been advanced (and so can
/// be tried for a different type).
...
// (Provide non-constant lvalue and rvalue ref functions so that we only accept
// explicit string_views but not implicitly converted ones)
inline uint64_t extract_unsigned(std::string_view &&s) {
  return extract_unsigned(s);
}

// Fallback base case; we only get here if none of the partial specializations
// below work
template <typename T, typename SFINAE = void> struct bt_serialize {
  static_assert(!std::is_same_v<T, T>,
                "Cannot serialize T: unsupported type for bt serialization");
};

template <typename T, typename SFINAE = void> struct bt_deserialize {
  static_assert(
      !std::is_same_v<T, T>,
...
inline void bt_need_more(const std::string_view &s) {
  if (s.empty())
    throw bt_deserialize_invalid{
        "Unexpected end of string while deserializing"};
}
...
/// Deserializes a signed or unsigned 64-bit integer from a string.  Sets the
/// second bool to true iff the value read was negative, false if positive; in
/// either case the unsigned value is return in .first.  Throws an exception if
/// the read value doesn't fit in a int64_t (if negative) or a uint64_t (if
/// positive).  Removes consumed characters from the string_view.
...
struct bt_serialize<T, std::enable_if_t<std::is_integral_v<T>>> {
  ...
  void operator()(std::ostream &os, const T &val) {
    // Cast 1-byte types to a larger type to avoid iostream interpreting them as
    // single characters
...
struct bt_deserialize<T, std::enable_if_t<std::is_integral_v<T>>> {
  void operator()(std::string_view &s, T &val) {
    constexpr uint64_t umax =
        static_cast<uint64_t>(std::numeric_limits<T>::max());
    constexpr int64_t smin =
...
template <> struct bt_serialize<std::string_view> {
...
/// char * and string literals -- we allow serialization for convenience, but
/// not deserialization
template <> struct bt_serialize<char *> {
  void operator()(std::ostream &os, const char *str) {
    bt_serialize<std::string_view>{}(os, {str, std::strlen(str)});
  }
};
template <size_t N> struct bt_serialize<char[N]> {
  void operator()(std::ostream &os, const char *str) {
    bt_serialize<std::string_view>{}(os, {str, N - 1});
  }
};

/// Partial dict validity; we don't check the second type for serializability,
/// that will be handled via the base case static_assert if invalid.
...
/// Determines whether the given type looks like a compatible map (i.e. has
/// std::string keys) that we can insert into.
template <typename T, typename = void>
...
template <typename T>
constexpr bool is_bt_output_dict_container =
    is_bt_output_dict_container_impl<T>::value;
template <typename T>
constexpr bool is_bt_input_dict_container =
    is_bt_output_dict_container_impl<T>::value;
...
/// Specialization for a dict-like container (such as an unordered_map).  We
/// accept anything for a dict that is const iterable over something that looks
/// like a pair with std::string for first value type.  The value (i.e. second
/// element of the pair) also must be serializable.
template <typename T>
  ...
  void operator()(std::string_view &s, T &dict) {
    ...
    while (!s.empty() && s[0] != 'e') {
      ...
      dict.insert(dict.end(),
                  typename T::value_type{std::move(key), std::move(val)});
    }
    if (s.empty())
      throw bt_deserialize_invalid("Deserialization failed: encountered end of "
                                   "string before dict was finished");
    s.remove_prefix(1); // Consume the 'e'
  }
};

/// Accept anything that looks iterable; value serialization validity isn't
/// checked here (it fails via the base case static assert).
template <typename T, typename = void>
...
struct is_bt_output_list_container_impl<
    T, std::enable_if_t<!std::is_same_v<T, std::string> &&
                        !is_bt_output_dict_container<T> && is_bt_insertable<T>>>
    : std::true_type {};

template <typename T>
constexpr bool is_bt_output_list_container =
    is_bt_output_list_container_impl<T>::value;
template <typename T>
constexpr bool is_bt_input_list_container =
    is_bt_input_list_container_impl<T>::value;
...
struct bt_deserialize<T, std::enable_if_t<is_bt_output_list_container<T>>> {
  ...
  void operator()(std::string_view &s, T &list) {
    ...
    if (s.empty())
      throw bt_deserialize_invalid("Deserialization failed: encountered end of "
                                   "string before list was finished");
    s.remove_prefix(1); // Consume the 'e'
  }
};
...
private:
  ...
  void operator()(std::string_view &s, Tuple<T...> &elems,
    ...
    if (s[0] != 'l')
      throw bt_deserialize_invalid_type(
          "Deserialization of tuple failed: expected 'l', found '"s + s[0] +
          "'"s);
    s.remove_prefix(1);
    (bt_deserialize<T>{}(s, std::get<Is>(elems)), ...);
    if (s.empty())
      throw bt_deserialize_invalid("Deserialization failed: encountered end of "
                                   "string before tuple was finished");
    if (s[0] != 'e')
      throw bt_deserialize_invalid("Deserialization failed: expected end of "
                                   "tuple but found something else");
    s.remove_prefix(1); // Consume the 'e'
  }
...
template <typename... T>
struct bt_deserialize<std::tuple<T...>>
    : bt_deserialize_tuple<std::tuple, T...> {};
...
template <typename T> inline constexpr bool is_bt_tuple = false;
template <typename... T>
...
// General template and base case; this base will only actually be invoked when
// Ts... is empty, which means we reached the end without finding any variant
// type capable of holding the value.
...
struct bt_deserialize_try_variant_impl {
  void operator()(std::string_view &, Variant &) {
    throw bt_deserialize_invalid("Deserialization failed: could not "
                                 "deserialize value into any variant type");
  }
};
...
struct bt_deserialize_try_variant_impl<
    std::enable_if_t<is_bt_deserializable<T>>, Variant, T, Ts...> {
...
// Deserialization to a variant; at least one variant type must be
// bt-deserializble.
template <typename... Ts>
...
template <> struct bt_serialize<bt_value> : bt_serialize<bt_variant> {};

template <> struct bt_deserialize<bt_value> {
  void operator()(std::string_view &s, bt_value &val);
};

template <typename T> struct bt_stream_serializer {
  const T &val;
...
/// Returns a wrapper around a value reference that can serialize the value
/// directly to an output stream.  This class is intended to be used inline
/// (i.e. without being stored) as in:
///
...
///     std::cout << encoded;
///
/// this approach is not generally recommended: the returned object stores a
/// reference to the passed-in type, which may not survive.  If doing this note
/// that it is the caller's responsibility to ensure the serializer is not used
/// past the end of the lifetime of the value being serialized.
///
/// Also note that serializing directly to an output stream is more efficient as
/// no intermediate string containing the entire serialization has to be
/// constructed.
///
template <typename T>
...
///
/// This takes any serializable type: integral types, strings, lists of
/// serializable types, and string->value maps of serializable types.
template <typename T> std::string bt_serialize(const T &val) {
...
///
template <typename T> T bt_deserialize(std::string_view s) {
  T val;
  bt_deserialize(s, val);
  return val;
}

/// Deserializes the given value into a generic `bt_value` type (wrapped
/// std::variant) which is capable of holding all possible BT-encoded values
/// (including recursion).
///
/// Example:
///
...
///     int v = get_int<int>(val); // fails unless the encoded value was
///     actually an integer that
///                                // fits into an `int`
///
...
/// Helper functions to extract a value of some integral type from a bt_value
/// which contains either a int64_t or uint64_t.  Does range checking, throwing
/// std::overflow_error if the stored value is outside the range of the target
/// type.
///
/// Example:
///
...
///     auto val = bt_get(encoded);
///     auto v = get_int<uint32_t>(val); // throws if the decoded value doesn't
///     fit in a uint32_t
...
IntType get_int(const bt_value &v) {
  if (auto *value = std::get_if<uint64_t>(&v)) {
    if constexpr (!std::is_same_v<IntType, uint64_t>)
      if (*value > static_cast<uint64_t>(std::numeric_limits<IntType>::max()))
        throw std::overflow_error("Unable to extract integer value: stored "
                                  "value is too large for the requested type");
  ...
  if constexpr (!std::is_same_v<IntType, int64_t>)
    if (value > static_cast<int64_t>(std::numeric_limits<IntType>::max()) ||
      ...
      throw std::overflow_error("Unable to extract integer value: stored value "
                                "is outside the range of the requested type");
  return static_cast<IntType>(value);
}
...
/// Converts a bt_list into the given template std::tuple or std::pair.  Throws
/// a std::invalid_argument if the list has the wrong size or wrong element
/// types.  Supports recursion (i.e. if the tuple itself contains tuples or
/// pairs).  The tuple (or nested tuples) may only contain integral types,
/// strings, string_views, bt_list, bt_dict, and tuples/pairs of those.
template <typename Tuple> Tuple get_tuple(const bt_list &x) {
  Tuple t;
  detail::get_tuple_impl(t, x,
                         std::make_index_sequence<std::tuple_size_v<Tuple>>{});
  return t;
}
template <typename Tuple> Tuple get_tuple(const bt_value &x) {
  return get_tuple<Tuple>(
      var::get<bt_list>(static_cast<const bt_variant &>(x)));
}

namespace detail {
template <typename T, typename It> void get_tuple_impl_one(T &t, It &it) {
  ...
  if constexpr (std::is_integral_v<T>) {
    ...
    // If we request a string/string_view, we might have the other one and need
    // to copy/view it.
...
void get_tuple_impl(Tuple &t, const bt_list &l, std::index_sequence<Is...>) {
  if (l.size() != sizeof...(Is))
    throw std::invalid_argument{
        "Unable to convert tuple: bt_list has wrong size"};
  auto it = l.begin();
...
/// Class that allows you to walk through a bt-encoded list in memory without
/// copying or allocating memory.  It accesses existing memory directly and so
/// the caller must ensure that the referenced memory stays valid for the
/// lifetime of the bt_list_consumer object.
...
public:
  bt_list_consumer(std::string_view data_) : data{std::move(data_)} {
    if (data.empty())
      throw std::runtime_error{
          "Cannot create a bt_list_consumer with an empty string_view"};
    if (data[0] != 'l')
      throw std::runtime_error{
          "Cannot create a bt_list_consumer with non-list data"};
    data.remove_prefix(1);
  }
  /// Copy constructor.  Making a copy copies the current position so can be
  /// used for multipass iteration through a list.
  ...
  bool is_negative_integer() const {
    return is_integer() && data.size() >= 2 && data[1] == '-';
  }
  /// Returns true if the next element looks like an encoded non-negative
  /// integer
  ...
  /// Attempt to parse the next value as a string (and advance just past it).
  ...
  std::string_view consume_string_view() {
    if (data.empty())
      throw bt_deserialize_invalid{
          "expected a string, but reached end of data"};
    else if (!is_string())
      throw bt_deserialize_invalid_type{"expected a string, but found "s +
                                        data.front()};
  ...
  /// Attempts to parse the next value as an integer (and advance just past it).
  /// Throws if the next value is not an integer.
  template <typename IntType> IntType consume_integer() {
    if (!is_integer())
  ...
  /// Consumes a list, return it as a list-like type.  Can also be used for
  /// tuples/pairs.  This typically requires dynamic allocation, but only has to
  /// parse the data once.  Compare with consume_list_data() which allows
  /// alloc-free traversal, but requires parsing twice (if the contents are to
  /// be used).
  template <typename T = bt_list> T consume_list() {
    T list;
    consume_list(list);
    return list;
  }
  ...
  template <typename T> void consume_list(T &list) {
    if (!is_list())
  ...
  /// Consumes a dict, return it as a dict-like type.  This typically requires
  /// dynamic allocation, but only has to parse the data once.  Compare with
  /// consume_dict_data() which allows alloc-free traversal, but requires
  /// parsing twice (if the contents are to be used).
  template <typename T = bt_dict> T consume_dict() {
    T dict;
    consume_dict(dict);
    return dict;
  }
  ...
  template <typename T> void consume_dict(T &dict) {
    if (!is_dict())
  ...
  /// Attempts to parse the next value as a list and returns the string_view
  /// that contains the entire thing.  This is recursive into both lists and
  /// dicts and likely to be quite inefficient for large, nested structures
  /// (unless the values only need to be skipped but aren't separately needed).
  /// This, however, does not require dynamic memory allocation.
  std::string_view consume_list_data() {
    ...
    while (!is_finished()) {
      skip_value();
      if (data.empty())
        throw bt_deserialize_invalid{"bt list consumption failed: hit the end "
                                     "of string before the list was done"};
    }
  ...
  /// Attempts to parse the next value as a dict and returns the string_view
  /// that contains the entire thing.  This is recursive into both lists and
  /// dicts and likely to be quite inefficient for large, nested structures
  /// (unless the values only need to be skipped but aren't separately needed).
  /// This, however, does not require dynamic memory allocation.
  std::string_view consume_dict_data() {
    ...
    while (!is_finished()) {
      ...
      if (!data.empty())
        skip_value();
      if (data.empty())
        throw bt_deserialize_invalid{"bt dict consumption failed: hit the end "
                                     "of string before the dict was done"};
    }
...
/// Class that allows you to walk through key-value pairs of a bt-encoded dict
/// in memory without copying or allocating memory.  It accesses existing memory
/// directly and so the caller must ensure that the referenced memory stays
/// valid for the lifetime of the bt_dict_consumer object.
class bt_dict_consumer : private bt_list_consumer {
  std::string_view key_;

  /// Consume the key if not already consumed and there is a key present (rather
  /// than 'e'). Throws exception if what should be a key isn't a string, or if
  /// the key consumes the entire data (i.e. requires that it be followed by
  /// something).  Returns true if the key was consumed (either now or
  /// previously and cached).
  bool consume_key() {
    if (key_.data())
      return true;
    if (data.empty())
      throw bt_deserialize_invalid_type{
          "expected a key or dict end, found end of string"};
    if (data[0] == 'e')
      return false;
  ...
  /// Clears the cached key and returns it.  Must have already called
  /// consume_key directly or indirectly via one of the `is_{...}` methods.
...
public:
  bt_dict_consumer(std::string_view data_) {
    ...
    if (data.empty())
      throw std::runtime_error{
          "Cannot create a bt_dict_consumer with an empty string_view"};
  ...
  bool is_string() {
    return consume_key() && data.front() >= '0' && data.front() <= '9';
  }
  /// Returns true if the next element looks like an encoded integer
  bool is_integer() { return consume_key() && data.front() == 'i'; }
  /// Returns true if the next element looks like an encoded negative integer
  bool is_negative_integer() {
    return is_integer() && data.size() >= 2 && data[1] == '-';
  }
  /// Returns true if the next element looks like an encoded non-negative
  /// integer
  ...
  /// Returns the key of the next pair.  This does not have to be called; it is
  /// also returned by all of the other consume_* methods.  The value is cached
  /// whether called here or by some other method; accessing it multiple times
  /// simple accesses the cache until the next value is consumed.
  std::string_view key() {
    if (!consume_key())
      throw bt_deserialize_invalid{
          "Cannot access next key: at the end of the dict"};
    return key_;
  }

  /// Attempt to parse the next value as a string->string pair (and advance just
  /// past it).  Throws if the next value is not a string.
  ...
  /// Attempts to parse the next value as an string->integer pair (and advance
  /// just past it). Throws if the next value is not an integer.
  template <typename IntType>
  ...
  /// Consumes a string->list pair, return it as a list-like type.  This
  /// typically requires dynamic allocation, but only has to parse the data
  /// once.  Compare with consume_list_data() which allows alloc-free traversal,
  /// but requires parsing twice (if the contents are to be used).
  template <typename T = bt_list> std::pair<std::string_view, T> next_list() {
    std::pair<std::string_view, T> pair;
    pair.first = next_list(pair.second);
    return pair;
  }

  /// Same as above, but takes a pre-existing list-like data type.  Returns the
  /// key.
  template <typename T> std::string_view next_list(T &list) {
    if (!is_list())
      throw bt_deserialize_invalid_type{"next bt value is not a list"};
  ...
  /// Consumes a string->dict pair, return it as a dict-like type.  This
  /// typically requires dynamic allocation, but only has to parse the data
  /// once.  Compare with consume_dict_data() which allows alloc-free traversal,
  /// but requires parsing twice (if the contents are to be used).
  template <typename T = bt_dict> std::pair<std::string_view, T> next_dict() {
    std::pair<std::string_view, T> pair;
    pair.first = next_dict(pair.second);
    return pair;
  }

  /// Same as above, but takes a pre-existing dict-like data type.  Returns the
  /// key.
  template <typename T> std::string_view next_dict(T &dict) {
    if (!is_dict())
      throw bt_deserialize_invalid_type{"next bt value is not a dict"};
  ...
  /// Attempts to parse the next value as a string->list pair and returns the
  /// string_view that contains the entire thing.  This is recursive into both
  /// lists and dicts and likely to be quite inefficient for large, nested
  /// structures (unless the values only need to be skipped but aren't
  /// separately needed).  This, however, does not require dynamic memory
  /// allocation.
  ...
  /// Same as next_list_data(), but wraps the value in a bt_list_consumer for
  /// convenience
  std::pair<std::string_view, bt_list_consumer> next_list_consumer() {
    return next_list_data();
  }

  /// Attempts to parse the next value as a string->dict pair and returns the
  /// string_view that contains the entire thing.  This is recursive into both
  /// lists and dicts and likely to be quite inefficient for large, nested
  /// structures (unless the values only need to be skipped but aren't
  /// separately needed).  This, however, does not require dynamic memory
  ...
  /// Same as next_dict_data(), but wraps the value in a bt_dict_consumer for
  /// convenience
  std::pair<std::string_view, bt_dict_consumer> next_dict_consumer() {
    return next_dict_data();
  }

  /// Skips ahead until we find the first key >= the given key or reach the end
  /// of the dict. Returns true if we found an exact match, false if we reached
  /// some greater value or the end. If we didn't hit the end, the next
  /// `consumer_*()` call will return the key-value pair we found (either the
  /// exact match or the first key greater than the requested key).
  ///
  /// Two important notes:
  ///
  /// - properly encoded bt dicts must have lexicographically sorted keys, and
  /// this method assumes
  ///   that the input is correctly sorted (and thus if we find a greater value
  ///   then your key does not exist).
  /// - this is irreversible; you cannot returned to skipped values without
  /// reparsing.  (You *can*
  ///   however, make a copy of the bt_dict_consumer before calling and use the
  ///   copy to return to the pre-skipped position).
  ...
  /// The `consume_*` functions are wrappers around next_whatever that discard
  /// the returned key.
  ///
  ...
  template <typename IntType> auto consume_integer() {
    return next_integer<IntType>().second;
  }

  template <typename T = bt_list> auto consume_list() {
    return next_list<T>().second;
  }

  template <typename T> void consume_list(T &list) { next_list(list); }

  template <typename T = bt_dict> auto consume_dict() {
    return next_dict<T>().second;
  }

  template <typename T> void consume_dict(T &dict) { next_dict(dict); }
...
inline uint64_t extract_unsigned(std::string_view &s) {
  ...
  while (!s.empty() && (s[0] >= '0' && s[0] <= '9')) {
    ...
    if (bigger < uval) // overflow
      throw bt_deserialize_invalid("Integer deserialization failed: value is "
                                   "too large for a 64-bit int");
    uval = bigger;
  }
  if (!once)
...
inline void
bt_deserialize<std::string_view>::operator()(std::string_view &s,
                                             std::string_view &val) {
  if (s.size() < 2)
    throw bt_deserialize_invalid{
  ...
  if (s.empty() || s[0] != ':')
    throw bt_deserialize_invalid{
        "Did not find expected ':' during string deserialization"};
  s.remove_prefix(1);

  if (len > s.size())
    throw bt_deserialize_invalid{
        "String deserialization failed: encoded string length is longer than "
        "the serialized data"};
...
// Check that we are on a 2's complement architecture.  It's highly unlikely
// that this code ever runs on a non-2s-complement architecture (especially
// since C++20 requires a two's complement signed value behaviour), but check at
// compile time anyway because we rely on these relations below.
...
inline std::pair<some64, bool> bt_deserialize_integer(std::string_view &s) {
  ...
  if (s.empty())
    throw bt_deserialize_invalid("Integer deserialization failed: encountered "
                                 "end of string before integer was finished");
  if (s[0] != 'e')
    throw bt_deserialize_invalid(
        "Integer deserialization failed: expected digit or 'e', found '"s +
        s[0] + '\'');
  s.remove_prefix(1);
...
inline void bt_deserialize<bt_value, void>::operator()(std::string_view &s,
                                                       bt_value &val) {
  if (s.size() < 2)
    throw bt_deserialize_invalid("Deserialization failed: end of string found "
                                 "where bt-encoded value expected");
